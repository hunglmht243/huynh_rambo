...
class MOMO
{
    ...
    public function SendMoney($receiver,$amount = 100,$comment = "",$requestkeyRaw="",$requestkey="")
    {
        ...
        // $result = $this->M2M_VALIDATE_MSG($receiver, $comment);
        // print_r($result);
        ...
        //     );
        // }
        //$message = $result['momoMsg']['message'];
        $this->send = array(
            "amount" => (int)$amount,
            "comment"=> $comment,
            "receiver"=> $receiver,
   ...
   public function CheckHistory($hours)
    {
        $requestkeyRaw = $this->generateRandom(32); 
    ...
    public function QUERY_TRAN_HIS_MSG_NEWV2($hours = 2)
    {
        $begin = (time() - (3600 * $hours)) * 1000;
        $header = array(
            "authorization: Bearer " . $this->config["authorization"],
    ...
    private function M2MU_CONFIRM($ID,$tranHisMsgs,$requestkeyRaw,$requestkey)
    {
        ...
        $Data =  array(
            ...
            'momoMsg' => 
                array(
                    ...
                    'desc'=> $tranHisMsgs['desc'],
                    'error'=>0,
                    'tranType'=>$tranHisMsgs['tranType'],
                    'ids' => 
                        array (
                            0 => $ID,
                        ),
    ...
    public function M2MU_INIT($requestkeyRaw,$requestkey)
    {
        $ipaddress = $this->get_ip_address();
        $microtime = $this->get_microtime();
        //$requestkeyRaw = $this->generateRandom(32);
    ...
    {    
        ...
        foreach ($tranHisMsg as $value){
            ...
            $if_transId= $this->connect->query("SELECT * FROM ( SELECT tranId FROM `momo_history` order by id DESC limit 60) AS T WHERE `tranId` = '".$value['transId']."' ");
            //print_r($if_transId->num_rows>0);
            if($if_transId->num_rows == 0){ // nếu chưa tồn tại trans_id thì mới get comment
                //$value= $tranHisMsg[0];
                $transId= $value['transId'];
                if ($value['io'] ==1){
                    $comment= $this->GET_COMMENT($transId);
                    $return[] = array(
                        'ID' => $value['id'],
                        "tranId"=> $value["transId"],
                        // 'io' => $value["io"],
                        "partnerId"=> $value["sourceId"],
                        //"status"=> $value["status"],
                        "partnerName" => empty($value["sourceName"]) ? "" : $value['sourceName'] ,
                        "amount" => (int)str_replace('.0','',$value["totalAmount"]),
                        "comment" => $comment,
                        //"desc"  => empty($value["errorDesc"]) ? "" : $value["errorDesc"],
                        //"millisecond" => empty($value["createdAt"]) ? 0 : $value['createdAt'] 
                    );
                } 
                //else $comment = 'khong lay';
    ...
    public function GET_COMMENT($transId){
        $requestkeyRaw = $this->generateRandom(32);
        $requestkey = $this->RSA_Encrypt($this->config["RSA_PUBLIC_KEY"],$requestkeyRaw);
        $data_post['url'] = "https://api.momo.vn/sync/transhis/details";
        $header = array(
            "Userid: ".$this->config["phone"],
            'Requestkey: '.$requestkey,
            "authorization: Bearer ".$this->config["authorization"],
        ...
        $response = $this->CURL("GET_COMMENT",$header,$this->Encrypt_data($Data,$requestkeyRaw));

        if(isset(json_decode($response['momoMsg']['serviceData'],true)['COMMENT_VALUE'])){
            $comment= json_decode($response['momoMsg']['serviceData'],true)['COMMENT_VALUE'];
        }
        else $comment =  'không có nội dung';
        return $comment;
    }
    ...
    public function CHECK_USER_PRIVATE($receiver,$requestkeyRaw,$requestkey)
    {
        $microtime = $this->get_microtime();
        //$requestkeyRaw = $this->generateRandom(32);
        //$requestkey = $this->RSA_Encrypt($this->config["RSA_PUBLIC_KEY"],$requestkeyRaw);
    ...
    public function RSA_Encrypt($key,$content,$action='')
    {
        if(empty($this->rsa)){
            $this->INCLUDE_RSA($key,$action);
        } 
        return base64_encode($this->rsa->encrypt($content));
    }

    private function INCLUDE_RSA($key,$action)
    {
